...
"""
Wine Quality Model Calibration Script - Robust Version
Looks for models in multiple possible locations
"""
...
def find_models_directory():
    ...
    possible_paths = [
        ...
        'labs/Github_labs/Lab2/models',  # From repo root
        '../../../models',  # Up from src
        os.path.join(os.getcwd(), 'models'),  # Absolute current
    ]
...
def load_latest_model():
    """
    Load the most recent trained model from any valid location
    
    Returns:
        Tuple of (model, timestamp, models_dir)
    """
    models_dir = find_models_directory()
    
    if models_dir is None:
        raise FileNotFoundError("No model files found in any location!")
    ...
    return model, timestamp, models_dir


def load_test_data(timestamp, models_dir):
    """
    Load test data for calibration evaluation
    ...
    Returns:
        Tuple of (X_test, y_test)
    """
    test_data_file = os.path.join(models_dir, f"test_data_{timestamp}.json")
    
    if not os.path.exists(test_data_file):
        print(f"Test data file not found: {test_data_file}")
        raise FileNotFoundError(f"Test data not found for timestamp {timestamp}")
...
def plot_calibration_curve(original_model, calibrated_model, X_test, y_test, timestamp, metrics_dir):
    """
    ...
    plot_filename = os.path.join(metrics_dir, f"calibration_curve_{timestamp}.png")
    plt.savefig(plot_filename, dpi=300, bbox_inches='tight')
    plt.close()
...
def save_calibration_results(calibrated_model, calibration_metrics, original_timestamp, models_dir):
    """
    Save calibrated model components and metrics
    """
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    metrics_dir = models_dir.replace('models', 'metrics')
    os.makedirs(metrics_dir, exist_ok=True)
    
    # Save model components
    try:
        base_model_file = os.path.join(models_dir, f"wine_quality_base_model_{timestamp}.pkl")
        calibrator_file = os.path.join(models_dir, f"wine_quality_calibrator_{timestamp}.pkl")
        
        joblib.dump(calibrated_model.base_model, base_model_file)
        joblib.dump(calibrated_model.calibrator, calibrator_file)
        
        print(f"\nBase model saved: {base_model_file}")
        print(f"Calibrator saved: {calibrator_file}")
    except Exception as e:
        print(f"\nModel saving note: {e}")
        print("   Calibration was successful! Metrics are saved.")
    
    # Save metrics
    metrics_file = os.path.join(metrics_dir, f"calibration_metrics_{timestamp}.json")
    calibration_data = {
        'original_model_timestamp': original_timestamp,
        'calibration_timestamp': timestamp,
        'calibration_method': 'isotonic_regression',
    ...
    with open(metrics_file, 'w') as f:
        json.dump(calibration_data, f, indent=4)
    print(f"Calibration metrics saved: {metrics_file}")
...
def main():
    ...
    # Load model (checks multiple locations)
    model, timestamp, models_dir = load_latest_model()
    
    # Load test data
    X_test, y_test = load_test_data(timestamp, models_dir)
    ...
    # Generate calibration curve
    metrics_dir = models_dir.replace('models', 'metrics')
    plot_calibration_curve(model, calibrated_model, X_test, y_test, timestamp, metrics_dir)
    
    # Save results
    save_calibration_results(calibrated_model, calibration_metrics, timestamp, models_dir)
